...
#| eval: false
fit_flux_all <-
  as_cmdstan_fit(list.files("chains/flux_all_simpl_Ksite_nohyper/", full.names = TRUE))
site_plot_num <- data |>
...
int_p <- delta |>
  ...
  inner_join(kappa, by = join_by(Site, sitenum, iter)) |>
  group_by(Site, sitenum, plot_new, plotnum, iter) |>
  mutate(
    value = integrate(
      function(x) delta * (exp(1 - x / tau) * x / tau)^k,
      0, Inf
    )$value,
    variable = "int_p"
  ) |>
...
eq_t95_p <- function(t, tau, k, thresh = 0.9) {
  abs(integrate(function(x) (exp(1 - x / tau) * x / tau)^k, 0, t)$value -
    thresh * integrate(
          function(x) (exp(1 - x / tau) * x / tau)^k,
          0, Inf
        )$value)
}
t90_p <- tau |>
  inner_join(kappa, by = join_by(Site, sitenum, iter)) |>
  group_by(Site, iter) |>
  mutate(value = optimize(
    function(x) eq_t95_p(x, tau, k),
    c(0, 100)
  )$minimum) |>
  group_by(Site) |>
  summarise(
  ...
  mutate(variable = "t90_p") |>
  select(Site, variable, mean, median, q5, q95)
t90_m <- lambda |>
  mutate(value = log(10) / lambda) |>
  group_by(Site) |>
  summarise(
    mean = mean(value),
    median = median(value),
...
environ |>
  ...
  inner_join(filter(params, variable == "t90_p"), by = join_by(site == Site)) |>
  ggplot(aes(pr, median, col = site)) +
  geom_pointrange(aes(ymin = q5, ymax = q95)) +
  labs(
...
read.csv("data/derived_data/aggregated_data_v9.csv") |>
  ...
  mutate(intensity = (ba_0 - ba_min) / ba_0) |>
  inner_join(filter(params, variable == "int_m")) |>
  ggplot(aes(intensity * 100, median, col = Site)) +
  geom_pointrange(aes(ymin = q5, ymax = q95)) +
  labs(
    x = "Logging intensity [% BA loss]",
