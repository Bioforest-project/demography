...
parameters {
  ...
  // productivity stp kurtosis (common for all sites)
  real<lower=k_bounds[1], upper=k_bounds[2]> k; 
  // flux rate (site)
...
transformed parameters {
  vector[n_log] mu_in_log = omega_s[site_log] .* y_log + 
    delta_p[plot_log] .* pow((time-dt./2) ./ tau_s[site_log], k) .*
    exp(k .* (1 - (time-dt./2) ./ tau_s[site_log]));
