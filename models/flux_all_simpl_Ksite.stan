...
parameters {
  ...
  // productivity stp kurtosis (site)
  real<lower=k_bounds[1], upper=k_bounds[2]> mu_k; 
...
transformed parameters {
  vector[n_log] k_obs = k_s[site_log];
  // vector[n_log] mu_in_log = omega_s[site_log] .* y_log + 
  //   delta_p[plot_log] .*(((time-dt./2) .* (time-dt./2)) ./ (tau_s[site_log].*tau_s[site_log]) .*
  //   exp(2 .* (1 - (time-dt./2) ./ tau_s[site_log])));
  vector[n_log] mu_in_log = omega_s[site_log] .* y_log + 
    delta_p[plot_log] .* pow((time-dt./2) ./ tau_s[site_log], k_obs) .*
    exp(k_obs .* (1 - (time-dt./2) ./ tau_s[site_log]));
  vector[n_log] mu_out_log = omega_s[site_log] .* y_log + alpha_p[plot_log] .* 
    exp(-lambda_s[site_log].*(time-dt./2));
  // vector[p_log] diff_log = exp(2) ./ 4 .* delta_p .* tau_s[site_plot] - 
  //   alpha_p ./ lambda_s[site_plot];
}
model {
  log(in_log) ~ normal(log(mu_in_log), sigma_in);
  log(out_log) ~ normal(log(mu_out_log), sigma_out);
  // diff_log ~ normal(0, sigma_y);
