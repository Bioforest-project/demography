data {
  ...
  vector[n_log] dt;  // census interval
...
transformed parameters {
  ...
  vector[n_log] mu_in_log = (
    ((delta_p[plot_log] .* exp(2) .* tau_s[site_log]) ./4 - 
    delta_p[plot_log] .* exp(2 .* (1 - time ./ tau_s[site_log])) .* 
    (time .* time ./ (2 .* tau_s[site_log]) + time ./ 2 + tau_s[site_log] ./ 4)) -
    ((delta_p[plot_log] .* exp(2) .* tau_s[site_log]) ./4 - 
    delta_p[plot_log] .* exp(2 .* (1 - (time - dt) ./ tau_s[site_log])) .* 
    ((time - dt) .* (time - dt) ./ (2 .* tau_s[site_log]) + (time - dt) ./ 2 + tau_s[site_log] ./ 4)) +
    
    omega_s[site_log] .* (
      delta_p[plot_log] .* exp(2 .* (1 - time ./ tau_s[site_log])) .*
      tau_s[site_log] ./4 .*  ((time .* time./tau_s[site_log] +
      2.*time + tau_s[site_log].*3./2)) +
      time .* (y0_p[plot_log] + delta_p[plot_log] .* exp(2) .*
      tau_s[site_log] ./ 4 - alpha_p[plot_log] ./ lambda_s[site_log]) -
      alpha_p[plot_log] ./ (lambda_s[site_log].*lambda_s[site_log]) .*
      exp(-lambda_s[site_log].*time) -
      (delta_p[plot_log] .* exp(2 .* (1 - (time - dt) ./ tau_s[site_log])) .*
      tau_s[site_log] ./4 .*  (((time - dt) .* (time - dt)./tau_s[site_log] +
      2.*(time - dt) + tau_s[site_log].*3./2)) +
      (time - dt) .* (y0_p[plot_log] + delta_p[plot_log] .* exp(2) .*
      tau_s[site_log] ./ 4 - alpha_p[plot_log] ./ lambda_s[site_log]) -
      alpha_p[plot_log] ./ (lambda_s[site_log].*lambda_s[site_log]) .*
      exp(-lambda_s[site_log].*(time - dt)) )
      )
      ) ./ dt ;
      
    vector[n_log] mu_out_log = (
    alpha_p[plot_log] ./ lambda_s[site_log] .* exp(-lambda_s[site_log].*time) .* 
    (exp(lambda_s[site_log].*dt) - 1) +
