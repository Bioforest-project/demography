...
params_test <- list(
  ...
  theta0 = k0 * (thetainf_old * k),
  omega = omega_old * k / mort_adjust,
  ba0 = 20
)
...
We used the following parameter values: `r paste(paste(names(params_test), lapply(params_test, function(x) round(x, 2)), sep = " = "), collapse = "; ")`. However, this results in an asymptotic basal area of `r params_test$thetainf/params_test$omega` m$^2$, `r mort_adjust` times higher than the value in control plots. 
