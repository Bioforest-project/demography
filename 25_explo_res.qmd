...
relative to the pre-logging stock, ! not avalaible for Lesong, Sungai Lalang, Tapajos and and Ulu Muda, as the first year of census is after logging => remove them for the time being
but we could either consider using the stock estimated at infinite (but check before how well it compare to prelogging when we have both), or consider the stock of control (but see how variable it is within sites when we have both control and pre-logging inventories)
...
#| warning: false
data_explo %>%
  filter(variable == "int_p"& !(is.na(ba_0))) %>% 
  ...
  labs(
    ...
    y = "AGB overproductivity [% prelogging AGB]"
  ) +
  theme_minimal() +
  theme(legend.position = "bottom")
```
