...
* cimatic variables related to productivity => we see below that over-productivity has a longer duration in the drier sites (higher climate water deficit, dry season intensity and length, vapour pressure deficit (but this is driven by one outlier site) and lower evapotranspiration and precipitation). 
...
There seems to be a strong site effect, but the within site trends seems to show that relative-overmortality decreases with logging intensity as well.
...
I have no real hypothesis on the effect of environmental variables on the relative over-mortality, but I make some graph anyway, in case some people have.
NB: the environmental variables are available at the site level, so with 4 sites, I don't think we will be able to say much.
...
data_explo %>% 
  ...
  geom_errorbar(aes(ymin = q5 * 100/agb_0, 
                    ymax = q95 * 100/agb_0, 
                    col = Site,
                    shape = continent),
                    width = 0) +
  geom_errorbarh(aes(xmin = q5_omega, 
                     xmax = q95_omega, 
                     col = Site,
                     shape = continent), 
                     height = 0) +
...
I have no real hypothesis on the effect of environmental variables on the relative over-productivity, but I make some graphs anyway, in case some people have.
NB: the environmental variables are available at the site level, so with 4 sites, I don't think we will be able to say much.
