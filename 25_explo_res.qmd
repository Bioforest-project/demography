...
> TO DO: don't calculate logging intensity for Lesong, Sungai Lalang and Ulu Muda , as the first year of census is after logging
...
plot_info <- read_csv("data/derived_data/aggregated_data_v9.csv") %>%
  ...
  summarise(
    ...
    ba_min = min(value[Year > Year_of_harvest &
...
* environmental variables related to productivity => we see below that over-productivity has a longer duration in the drier sites (higher climate water deficit, dry season intensity and length, vapour pressure deficit and lower evapotranspiration and precipitation). 
...
relative to the pre-logging stock, ! not avalaible for Lesong, Sungai Lalang and Ulu Muda, as the first year of census is after logging => remove them for the time being
but we could either consider using the stock estimated at infinite (but check before how well it compare to prelogging when we have both), or consider the stock of control (but see how variable it is within sites when we have both control and pre-logging inventories)
