...
plot_info <- read_csv("data/derived_data/aggregated_data_v10.csv") |>
...
#| warning: false
data_explo |>
  filter(variable == "t90m") |>
  ggplot(aes(median, reorder(Site, median), col = continent)) +
  geom_pointrange(aes(xmin = q5, xmax = q95)) +
  labs(
    x = "Duration of over-mortality (t90, in years)",
    y = ""
  ) +
  theme_minimal()
```

We expect it to be quite consistent between sites =\> confirmed, although some differences between sites have in duration of over-mortality. This is not due to the timing of the censuses after logging (trec):

```{r}
#| message: false
#| warning: false
read_csv("data/derived_data/aggregated_data_v10.csv") |>
...
```{r}
data_explo |>
  filter(variable == "t90p") |>
  ggplot(aes(median, reorder(Site, median), col = continent)) +
  geom_pointrange(aes(xmin = q5, xmax = q95)) +
  labs(
    x = "Duration of over-productivity (t90, in years)",
    y = ""
  ) +
  theme_minimal()
```
...
- the ecology of the pioneer species (life span) =\> could that explain the effect of continent?

- climatic variables related to productivity =\> we see below that over-productivity has a longer duration in the drier sites (higher climate water deficit, dry season intensity and length, vapour pressure deficit (but this is driven by one outlier site) but the trend is quite weak, and this is not suported by evapotranspiration and precipitation. The pattern with soil humidity is also counter-intuitive (but soil humidity is very positively correlated with maximum temperature).
...
data_explo |>
  ...
  labs(
    x = "",
    y = "Duration of over-productivity (t90, in years)"
  ) +
  facet_wrap(~var_clim, scales = "free_x") +
  theme_bw() +
  theme(legend.position = "bottom")
...
- soil variables related to productivity =\> we see below that higher bulk density is (weakly) related to to longer duration of over-productivity, and over-productivity last less time in soil with higher clay content. This could support the hypotheses that over-productivity lasts longer in less fertile soils. But there is no pattern with SOC...
...
#| warning: false
data_explo |>
  filter(variable %in% c("omega", "t90p")) |>
  select(Site, variable, median, q5, q95, continent) |>
  pivot_wider(
    names_from = variable,
    values_from = c(median, q5, q95)
  ) |>
  ggplot(aes(x = median_omega, y = median_t90p)) +
  geom_errorbar(aes(
    ymin = q5_t90p, ymax = q95_t90p,
    col = continent,
    width = 0
  )) +
  geom_errorbarh(aes(
...
> The trends observed could support the hypothesis that over-productivity would lask longuer is less productive (drier and less fertile) and less dynamic sites (lower turnover rate), which would make sense. Let's be careful not to over interpret it though, as there are just 8 sites, the patterns are not very clear, and ther are possible confunding effects due to the correlation among environmental factors in the data set.
...
The graph below show that there is a lot of variability in the relative AGB over-mortality, with values going from less than 5% to more than 15%.

> The values of over-mortality are much more consistent than the ones from the previous version of the model.

```{r}
#| message: false
#| warning: false
data_explo |>
  filter(variable == "stpm" & !(is.na(agb_0))) |>
...
We expect that this variability would be partly explained by logging intensity, with a higher over-mortality when logging intensity is higher =\> this is not confirmed, as the graph below shows a weal opposite effect, or no effect...
...
- Climate variables: no clear trend...
...
- Forest dynamism (turnover rate $\omega$): the graph below shows that over-mortality decreases with increasing turnover rate.
...
The graph below show that there is a lot of variability in the relative AGB over-mortality, with values going from less than 10% to more than 50%.

```{r}
#| message: false
#| warning: false
data_explo |>
  filter(variable == "stpp" & !(is.na(agb_0))) |>
  mutate(site_plot = paste(Site, plot_new)) |>
  ggplot(aes(median * 100 / agb_0,
    reorder(site_plot, median * 100 / agb_0),
    col = Site,
...
- Forest dynamism (turnover rate $\omega$): the graph below shows that over-productivity decreases with increasing turnover rate.
