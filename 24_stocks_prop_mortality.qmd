...
$$ K = ba_0 - \frac{\theta_{\infty}}{\omega} - \frac{\lambda(\theta_{\infty}-\theta_0) }{1-\omega\lambda} -\frac{16e^2\delta(\theta_{\infty}-\theta_0)}{\tau^2(2\omega-\tau)^3}$$ (see Appendix "Differential equation")
...
prod <- function(x,
                 ...
                 thetainf = 0.5,
                 theta0 = 0.4) {
  return(theta0 + (thetainf - theta0) * ((1 - exp(-lambda * x)) + delta *
                                           ((x / tau) * exp(1 - x / tau))^2))
}
mort <- function(x,
                 tau = 10,
...
get_t95 <- function(tau, lambda, delta, thetainf, theta0, omega, ba0) {
  min(which(
    ba(1:1e4, tau, lambda, delta, thetainf, theta0, omega, ba0) > 0.95 *
...
pars |>
  ...
  cbind(data.frame(
    tau = 10, lambda = 0.05, delta = 1, thetainf = 0.5,
